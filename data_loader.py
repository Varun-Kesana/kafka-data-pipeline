...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        csv_file_path='file:///' + file_path.split(".")[0] + '.csv'
        
        query = """LOAD CSV WITH HEADERS FROM $file_path AS row 
        WITH datetime(replace(row.tpep_pickup_datetime, ' ', 'T')) AS pickup_datetime, datetime(replace(row.tpep_pickup_datetime, ' ', 'T')) AS dropoff_datetime, toInteger(row.PULocationID) AS pickup_id, toInteger(row.DOLocationID) AS dropoff_id, toFloat(row.trip_distance) AS distance, toFloat(row.fare_amount) AS fare 
        MERGE (pickup:Location {name: pickup_id}) 
        MERGE (dropoff:Location {name: dropoff_id}) 
        CREATE (pickup)-[:TRIP {distance: distance, fare: fare, pickup_dt: pickup_datetime, dropoff_dt: dropoff_datetime}]->(dropoff)""" 
        ...
        with self.driver.session() as session:
            ...
            result = session.run(query, {"file_path": csv_file_path})
